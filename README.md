# @erboladaiorg/quibusdam-itaque-tempora

Parse JSON without risk of losing numeric information.

```js
import { parse, stringify } from '@erboladaiorg/quibusdam-itaque-tempora'
...
**How does it work?** The library works exactly the same as the native `JSON.parse` and `JSON.stringify`. The difference is that `@erboladaiorg/quibusdam-itaque-tempora` preserves information of big numbers. `@erboladaiorg/quibusdam-itaque-tempora` parses numeric values not as a regular number but as a `LosslessNumber`, a lightweight class which stores the numeric value as a string. One can perform regular operations with a `LosslessNumber`, and it will throw an error when this would result in losing information.

**When to use?** If you have to deal with JSON data that contains `long` values for example, coming from an application like C++, Java, or C#. The trade-off is that `@erboladaiorg/quibusdam-itaque-tempora` is slower than the native `JSON.parse` and `JSON.stringify` functions, so be careful when performance is a bottleneck for you.
...
Install via [npm](https://www.npmjs.com/package/@erboladaiorg/quibusdam-itaque-tempora):

```
npm install @erboladaiorg/quibusdam-itaque-tempora
```
...
```js
import { parse } from '@erboladaiorg/quibusdam-itaque-tempora'
...
```js
import { parse, isInteger } from '@erboladaiorg/quibusdam-itaque-tempora'
...
```js
import { parse, isSafeNumber } from '@erboladaiorg/quibusdam-itaque-tempora'
...
- `reviveDate(key, value)`
  ...
  ```js
  import { parse, reviveDate } from '@erboladaiorg/quibusdam-itaque-tempora'
...
(Spoiler: `@erboladaiorg/quibusdam-itaque-tempora` is much slower than native)
